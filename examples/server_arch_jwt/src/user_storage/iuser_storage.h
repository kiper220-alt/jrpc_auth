...
public:
    ...
    virtual std::optional<uint64_t> authenticate(const QString &username, const QString &password) = 0;

    /// @brief Just get user version
    /// @param username user name
    /// @return user version if user exists, otherwise std::nullopt
    virtual std::optional<uint64_t> getUserVersion(const QString &username) = 0;
